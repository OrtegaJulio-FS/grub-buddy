...
describe('core loop: create spot -> create log -> server-side aggregate correctness', () => {
  test('average_rating/log_count start null/0 and update as logs are added', async () => {
    ...
    const before = await request(app).get(`/spots/${spotId}`);
    expect(before.body.average_rating).toBeNull();
    expect(before.body.log_count).toBe(0);

    await request(app).post('/logs').send({ spotId, rating: 5 }).expect(201);
    await request(app).post('/logs').send({ spotId, rating: 3 }).expect(201);

    const after = await request(app).get(`/spots/${spotId}`);
  ...
  test('GET /spots list view carries the same aggregate as GET /spots/:id', async () => {
    const spotRes = await request(app).post('/spots').send({ name: 'List Cafe' });
    const spotId = spotRes.body.id;
    await request(app).post('/logs').send({ spotId, rating: 4 }).expect(201);

    const listRes = await request(app).get('/spots');
  ...
  test('a third log correctly shifts the average', async () => {
    const spotRes = await request(app).post('/spots').send({ name: 'Three Log Cafe' });
    const spotId = spotRes.body.id;

    await request(app).post('/logs').send({ spotId, rating: 5 }).expect(201);
    await request(app).post('/logs').send({ spotId, rating: 5 }).expect(201);
    await request(app).post('/logs').send({ spotId, rating: 2 }).expect(201);

    const res = await request(app).get(`/spots/${spotId}`);
